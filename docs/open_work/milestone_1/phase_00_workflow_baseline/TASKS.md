...
- [x] `M1-P00-T06` Establish the deterministic visual-QA foundation and isolated QA Android package.
  ...
  - Evidence: The versioned catalog reserves 26 IDs and directly renders 16 P00-reachable states; strict request/ready schemas carry nonce, scenario, seed, UI scale, safe-area provenance, APK hash, viewport, live safe area, assertions, errors, and deterministic state hash. Contract tests, all-16 direct headless execution, malformed-request envelopes, UI-scale echo/application checks, and five-run representative hash repetition passed under Godot 4.6.3. Normal APK `3839079c1d0e6a97b80e957eb36bfaace7e3b3179fd0647f45e40c837f075316` is `com.milin.geometory` with zero loadable QA/test resources; QA APK `9831b1f06b9df2a0521ca432cd745529b1c5281de8d50dff8e98d0c2299e76c2` is `com.milin.geometory.qa` with the fixture resources and zero tests; both request only `VIBRATE`. Emulator contract artifact `artifacts/visual_qa/20260719_123144/allocation_staged` passed request/ready, overlay, visible-window, fatal-log, and assertion checks while remaining explicitly uncertified visually. Alias-only physical evidence at `20260719_114056/movement_pending_path` proves package launch/readiness and retains no screenshot/log after correctly rejecting an unrelated overlay.

- [x] `M1-P00-T07` Reconcile P00 documentation and close phase hygiene.
  ...
  - Evidence: Authorities, archive, tracker regression, local suites, inspected APKs, emulator/physical contracts, and hygiene pass. Closeout commit `153efbc` passed GitHub Actions run `29698512789`; annotated tag `m1-p00` is published at that commit, the milestone branch/tag were fetch-verified, and the branch-only transition activates P01 without merging `main`.
